...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-0.5">Kelola produk stock Anda</p>
        </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 card-hover">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-blue-600 mt-0.5">{{ $products->count() }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 card-hover">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-green-600 mt-0.5">{{ $products->sum('stock') }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 card-hover">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-yellow-600 mt-0.5">{{ $products->where('stock', '<=', 5)->count() }}</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 card-hover">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm font-medium text-gray-400">Aktif</p>
                <p class="text-2xl font-bold text-green-600 mt-0.5">{{ $products->where('status', 'active')->count() }}</p>
            </div>
            <div class="stat-icon bg-green-50 text-green-500">✅</div>
        </div>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <tr class="table-row-hover transition">
                    ...
                    <td class="px-6 py-3.5 text-sm">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('client.stock.edit', $product->id) }}" 
                               class="text-blue-600 hover:text-blue-800 hover:underline font-medium">Edit</a>
                            <span class="text-gray-300">|</span>
                            <form action="{{ route('client.stock.destroy', $product->id) }}" method="POST" class="inline" 
    ...
    <!-- PAGINATION DIHAPUS KARENA PAKAI get() BUKAN paginate() -->
</div>
@endsection